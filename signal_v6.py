...
# Always save chart to file
output_file = f"signal_v6_{symbol.replace('/', '_')}_{timeframe}.png"
plt.savefig(output_file, dpi=150, bbox_inches='tight')
print(f"\n📈 Chart opgeslagen als: {output_file}")

# Print signal summary
buy_count = df['buy_signal'].notna().sum()
sell_count = df['sell_signal'].notna().sum()
print(f"✅ Analyse compleet: {buy_count} BUY signalen, {sell_count} SELL signalen")

# Try to show interactively (will be skipped in headless environments)
try:
    plt.show()
except Exception:
    pass
